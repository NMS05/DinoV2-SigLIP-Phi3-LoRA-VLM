...
Run with:
    ...
    LLAVA-1.5 repoduction final loss = {"Finetune/Loss": 0.7097251415252686, "Finetune/Loss (Raw)": 0.7241162657737732}
    For LLAVA-LRV = {"Finetune/Loss": 0.8132457733154297, "Finetune/Loss (Raw)": 0.9371241927146912}
"""

"""
1. phi3 prompter                                                    done!
2. phi3 model                                                       done!
3. integration with base_llm  - lora + configs                      done!
4. init for prompter, llm and update materialize.py                 done!
5. freeze parameter constraint in prismatic                         done!
6. update both dataset and model config files                       done!
7. update pretrain.py params and set stage in models
"""
...
class PretrainConfig:
    ...
    model: ModelConfig = field(
        default_factory=ModelConfig.get_choice_class(ModelRegistry.MY_MODEL.model_id)
    )                                                                                        #---------->>>>>>>----------->>>>>>>>>>>------>>>>>>> my changes!

    # DatasetConfig (`prismatic/conf/datasets.py`); override with --dataset.type `DatasetRegistry.<DATASET>.dataset_id`
    dataset: DatasetConfig = field(
        default_factory=DatasetConfig.get_choice_class(DatasetRegistry.LLAVA_LRV.dataset_id)
    )                                                                                        #---------->>>>>>>----------->>>>>>>>>>>------>>>>>>> my changes

    # Pretraining Stage in < align (projector-only) | finetune (projector + LLM) | full-finetune (all) >

    # stage: str = "align"                         # Pretraining Stage in < align | finetune >     #---------->>>>>>>----------->>>>>>>>>>>------>>>>>>> my changes!
    # pretrained_checkpoint: Optional[Path] = None # Pretrained Checkpoint to Load (for `finetune`)
    stage: str = "finetune"
    pretrained_checkpoint: Optional[Path] = "runs/llava-lrv+dino-siglip-phi3-lora-model+stage-align+x7/checkpoints/latest-checkpoint.pt"


    # Run Arguments
    run_id: Optional[str] = None                                    # Run ID for logging, Weights & Biases
    run_root_dir: Path = Path("runs") # Path to directory to store logs & checkpoints    #---------->>>>>>>----------->>>>>>>>>>>------>>>>>>> my changes!
    seed: int = 7                                                   # Random seed (for reproducibility)

    # HF Hub Credentials (for any gated models)
    # hf_token: Union[str, Path] = Path(".hf_token")                  # Environment variable or Path to HF Token    #---------->>>>>>>----------->>>>>>>>>>>------>>>>>>> my changes!
    hf_token = None

    # Tracking Parameters
    trackers: Tuple[str, ...] = ("jsonl", "wandb")   # Trackers to initialize (if W&B, add config!)
    wandb_project: str = "prismatic-phi3"       # Name of W&B project (default: `prismatic`)     #---------->>>>>>>----------->>>>>>>>>>>------>>>>>>> my changes!
...
def pretrain(cfg: PretrainConfig) -> None:
    ...
    # hf_token = cfg.hf_token.read_text().strip() if isinstance(cfg.hf_token, Path) else os.environ[cfg.hf_token]           #---------->>>>>>>----------->>>>>>>>>>>------>>>>>>> my changes!
