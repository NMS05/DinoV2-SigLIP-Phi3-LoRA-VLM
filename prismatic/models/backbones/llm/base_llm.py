...
from peft import get_peft_model                #---------->>>>>>>----------->>>>>>>>>>>------>>>>>>> my changes!
...
class HFCausalLLMBackbone(LLMBackbone, ABC):
    def __init__(
        ...
        if not self.inference_mode:
            ...
            if enable_peft == True: self.llm = get_peft_model(self.llm, lora_peft_config)        #---------->>>>>>>----------->>>>>>>>>>>------>>>>>>> my changes!
        ...
        # assert (self.tokenizer("Testing 123", add_special_tokens=True).input_ids[0] == self.tokenizer.bos_token_id) and (                               #---------->>>>>>>----------->>>>>>>>>>>------>>>>>>> my changes!
